...
export function ResourcesGrid() {
  ...
  return (
    <div className="grid gap-8">
      <Link href={`/resources/${featured.slug}`} className="group outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-300)]">
        <Card className="grid gap-6 overflow-hidden p-4 transition-transform group-hover:-translate-y-1 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="grid content-center gap-3 p-2">
            ...
            <p className="font-mono text-xs text-[var(--color-ink-faint)]">{featured.date}</p>
            <h2 className="text-[32px] font-normal leading-[1.15] tracking-[0px] text-[var(--foreground)]">{featured.title}</h2>
            <p className="text-[15px] leading-6 text-[var(--color-ink-muted)]">{featured.excerpt}</p>
            <span className="inline-flex items-center gap-2 text-sm text-[var(--color-ink-strong)]">
        ...
        {rest.map((post) => (
          <Card key={post.slug} className="grid gap-3 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="font-mono text-xs text-[var(--color-ink-faint)]">{post.date}</span>
            </div>
            <h3 className="text-xl font-medium tracking-[0px]">{post.title}</h3>
            <p className="text-sm leading-6 text-[var(--color-ink-muted)]">{post.excerpt}</p>
            <span className="text-sm text-[var(--color-ink-faint)]">Observed post card</span>
          </Card>
        ))}
      </div>
    </div>
