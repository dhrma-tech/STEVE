    ...
    <div className="grid gap-8 lg:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="lg:sticky lg:top-[116px] lg:self-start">
        <Dialog>
          <DialogTrigger asChild>
            <Button variant="ghost" className="w-full justify-between rounded-[14px] border border-[var(--border-10)] bg-[var(--background-l0)] p-4 lg:hidden">
        ...
        <div className="hidden rounded-[14px] border border-[var(--border-10)] bg-[var(--background-l0)] p-4 lg:block">
      ...
      <article className="grid gap-8">
        <div className="grid gap-5">
          <p className="font-mono text-xs uppercase tracking-[0.08em] text-[var(--foreground-50)]">{chapter.eyebrow}</p>
          <h1 className="text-[40px] font-normal leading-[1.15] tracking-[0px]">{chapter.title}</h1>
          <p className="max-w-[72ch] text-[16px] leading-7 text-[var(--foreground-60)]">{chapter.intro}</p>
...
function Toc({ chapter, active, className }: { chapter: HowToChapter; active: string; className?: string }) {
  ...
  return (
    <nav className={className} aria-label={`${chapter.title} table of contents`}>
      <div className="mb-4 h-1 overflow-hidden rounded-full bg-[var(--foreground-10)]">
        <div className="h-full bg-[var(--tt-brand-color-500)] transition-[width]" style={{ width: `${progress}%` }} />
      </div>
